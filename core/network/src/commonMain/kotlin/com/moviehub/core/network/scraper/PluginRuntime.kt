...
/**
 * QuickJS-based JavaScript plugin runtime for MovieHub.
 * Executes external .js scraper scripts inside a sandboxed QuickJS context,
 * providing native bridges for fetch, cheerio (HTML parsing via Ksoup),
 * and CryptoJS-compatible crypto operations.
 *
 * Ported and adapted from Nuvio's PluginRuntime.
 */
object PluginRuntime {
    ...
    /**
     * Execute a JavaScript plugin script and return parsed stream results.
     *
     * @param httpClient The Ktor HttpClient to use for fetch bridge requests.
     * @param code The JavaScript source code of the plugin scraper.
     * @param tmdbId The TMDB/IMDb ID of the content.
     * @param mediaType "movie" or "series"/"tv".
     * @param season Season number, or null for movies.
     * @param episode Episode number, or null for movies.
     * @param scraperId Identifier for logging purposes.
     * @param scraperSettings Optional key-value settings to pass to the JS context.
     * @return List of parsed PluginRuntimeResult items.
     */
    ...
    private suspend fun executePluginInternal(
        ...
        try {
            quickJs(Dispatchers.Default) {
                ...
                // Wrap and execute the plugin code
                val wrappedCode = """
                    var module = { exports: {} };
    ...
    private fun performNativeFetch(
        ...
        return try {
            ...
            val response = runBlocking {
                ...
                Triple(httpResponse.status, responseBody, responseHeaders)
            }

            val (status, responseBody, responseHeaders) = response
            val truncatedHeaders = responseHeaders.mapValues { (_, value) ->
                truncateString(value, MAX_FETCH_HEADER_VALUE_CHARS)
            }
            JsonObject(
                mapOf(
                    "ok" to JsonPrimitive(status.value in 200..299),
                    "status" to JsonPrimitive(status.value),
                    "statusText" to JsonPrimitive(status.description),
                    "url" to JsonPrimitive(url),
                    "body" to JsonPrimitive(truncateString(responseBody, MAX_FETCH_BODY_CHARS)),
                    "headers" to JsonObject(truncatedHeaders.mapValues { JsonPrimitive(it.value) }),
                ),
            ).toString()
    ...
    private fun buildPolyfillCode(scraperId: String, settingsJson: String): String {
        return """
            ...
            function __normalizeWordArrayInput(value) {
                ...
                if (value && typeof value === 'object' && typeof value.__hex === 'string') return __crypto_hex_to_utf8(value.__hex);
            ...
            var CryptoJS = {
                enc: {
                    Hex: {
                        stringify: function(wordArray) { return __wordArrayToHex(wordArray); },
                        parse: function(hexStr) { return __wordArrayFromHex(hexStr || ''); }
                    },
                    Utf8: {
                        ...
                        parse: function(text) { return __wordArrayFromUtf8(text); }
                    },
                    Base64: {
                        stringify: function(wordArray) {
                        ...
                        parse: function(base64) { return __wordArrayFromBase64(base64); }
                    }
                },
